  ...
  public:
        ...
        for (int i = 0; i < strs[0].size(); ++i) {
            ...
            for (int j = 1; j < size; ++j) {
                /*if ((i == strs[0].size())
                        ...
                        || (i >= strs[j].size())) {*/
                if ((i == strs[0].size()) || (strs[0][i] != strs[j][i])) {
    ...
    bool isIpv4(string &IP) {
        ...
        for (int i = 0; i < vStr.size(); ++i) {
            if ((vStr[i].size() == 0) || (vStr[i].size() < 0) || (vStr[i].size() > 3) || ((vStr[i][0] == '0') && (vStr[i].size() != 1))) {
                return false;
            }
    ...
    bool isIpv6(string &IP) {
        ...
        for (int i = 0; i < vStr.size(); ++i) {
            if ((vStr[i].size() == 0) || (vStr[i].size() > 4)) {
                return false;
            }

            for (int j = 0; j < vStr[i].size(); ++j) {
                if (!(isdigit(vStr[i][j]) || (vStr[i][j] >= 'a' && vStr[i][j] <= 'f') || (vStr[i][j] >= 'A' && vStr[i][j] <= 'F'))) {
                    return false;
                }
            }
        }
